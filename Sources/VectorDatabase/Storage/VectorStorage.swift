...
public final class VectorStorage: @unchecked Sendable {
    ...
    private func grow() {
        // `max(capacity * 2, 1)` only matters when `capacity == 0` (a freshly-loaded,
        // previously-empty on-disk database, per PersistenceManager.load()); for any
        // non-zero starting capacity this is identical to plain doubling.
