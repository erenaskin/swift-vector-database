...
final class FlatIndexTests: XCTestCase {
    ...
    func testDotProduct10kVectorsAgainstReference() throws {
        ...
        let vectors = TestFixtures.randomVectors(
            count: count, dimension: dim, seed: 0xDEAD_BEEF_0001)
    ...
    func testCosine10kVectorsAgainstReference() throws {
        ...
        let vectors = TestFixtures.randomUnitVectors(
            count: count, dimension: dim, seed: 0xDEAD_BEEF_0002)
        let index = try buildIndex(vectors: vectors, dimension: dim, metric: .cosine)

        let queries = TestFixtures.randomUnitVectors(
            count: 5, dimension: dim, seed: 0xCAFE_BABE_0002)
        for (qi, query) in queries.enumerated() {
            let got = TestFixtures.search(index: index, query: query, k: k)
    ...
    func testEuclidean10kVectorsAgainstReference() throws {
        ...
        let vectors = TestFixtures.randomVectors(
            count: count, dimension: dim, seed: 0xDEAD_BEEF_0003)
    ...
    func testRemoveNonExistentThrowsNotFound() {
        ...
        XCTAssertThrowsError(try index.remove(internalID: 99)) { error in
            guard case VectorDatabaseError.notFound = error else {
...
final class VectorMathAccelerateTests: XCTestCase {
    ...
    func testNearZeroTreatedAsZeroAndNoNaN() {
        ...
        v.withUnsafeMutableBufferPointer { VectorMath.normalize($0.baseAddress!, $0.count) }
        for e in v {
            XCTAssertTrue(e.isFinite, "normalize() on near-zero must not produce NaN/Inf")
        }
    }
    ...
    func testBatchDotMatchesPerVectorDot() {
        ...
        let batch = flat.withUnsafeBufferPointer { fp in
            query.withUnsafeBufferPointer { qp in
                VectorMath.batchDot(
                    query: qp.baseAddress!, vectors: fp.baseAddress!, count: n, dim: dim)
            }
        }
        for (i, v) in vectors.enumerated() {
            let perVec = query.withUnsafeBufferPointer { qp in
                v.withUnsafeBufferPointer { vp in
                    VectorMath.dot(qp.baseAddress!, vp.baseAddress!, dim)
                }
            }
    ...
    func testFlatIndexResultsUnchangedAfterAccelerateSwap() throws {
        ...
        let query = TestFixtures.randomVectors(count: 1, dimension: dim, seed: 0xCAFE_BABE_0001)
            .first!
        let got = TestFixtures.search(index: index, query: query, k: k)
        let ref = TestFixtures.referenceDotProduct(query: query, vectors: vectors, k: k)
        XCTAssertEqual(
            got.map { Int($0.id) }, ref.map { $0.index },
            "Phase 2 swap must not change FlatIndex top-k")
    }

    // MARK: - [DoD-F] Batched performance (PROMPT 1)

    func testBatchedSearchIsFaster() throws {
        let dim = 128
        let counts = [10, 100, 500, 2000]
        let query = TestFixtures.randomVectors(count: 1, dimension: dim, seed: 0xCAFE).first!
        let k = 10

        for count in counts {
            let vectors = TestFixtures.randomVectors(
                count: count, dimension: dim, seed: UInt64(count))
            var index = FlatIndex(dimension: dim, metric: .dotProduct)
            for (i, v) in vectors.enumerated() {
                try TestFixtures.insert(into: &index, internalID: Int32(i), vector: v)
            }

            // Simülasyon: Eski skaler yol (VectorMath.dot döngüsü + sort)
            // Bu tam olarak eski FlatIndex.search mantığının adil kopyasıdır.
            let flatVectors = vectors.flatMap { $0 }
            let scalarStart = CFAbsoluteTimeGetCurrent()
            var scalarResults: [(Int32, Float)] = []
            scalarResults.reserveCapacity(count)
            query.withUnsafeBufferPointer { qBuf in
                let qPtr = qBuf.baseAddress!
                flatVectors.withUnsafeBufferPointer { vBuf in
                    let vBase = vBuf.baseAddress!
                    for i in 0..<count {
                        let score = VectorMath.dot(qPtr, vBase + i * dim, dim)
                        scalarResults.append((Int32(i), score))
                    }
                }
            }
            scalarResults.sort { $0.1 > $1.1 }
            let _ = Array(scalarResults.prefix(k))
            let scalarTime = CFAbsoluteTimeGetCurrent() - scalarStart

            // Yeni batched yol (FlatIndex.search + O(N log k) heap)
            let batchedStart = CFAbsoluteTimeGetCurrent()
            let _ = TestFixtures.search(index: index, query: query, k: k)
            let batchedTime = CFAbsoluteTimeGetCurrent() - batchedStart

            print("Count: \(count) | Scalar: \(scalarTime)s | Batched: \(batchedTime)s")

            if count >= 500 {
                // Büyük koleksiyonlarda cblas_sgemv'nin %30-%50 gibi belirgin bir oranda daha hızlı olması beklenir.
                // CLI ortamlarındaki mikro-saniyelik dalgalanmaları (scheduling noise) tolere etmek için küçük bir sabit epsilon (+ 0.005) ekliyoruz.
                XCTAssertLessThan(
                    batchedTime, scalarTime * 0.7 + 0.005,
                    "Batched should be significantly faster for count \(count)")
            } else {
                // Küçük koleksiyonlarda (N=10, 100) BinaryHeap overhead'i nedeniyle daha yavaş olabilir.
                XCTAssertLessThan(
                    batchedTime, scalarTime * 5.0 + 0.010,
                    "Batched should not heavily regress for small count \(count)")
            }
        }
    }
}
