...
final class WALDurabilityTests: XCTestCase {
    ...
    func testFsyncDefaultIntervalRecoversWithoutSave() async throws {
        ...
        let recoveredDB = try VectorDatabase(dimension: 2, metric: .dotProduct, path: dbURL)
        ...
        XCTAssertEqual(
            liveCount, 1,
            "The record should be fully recoverable since fsync was called immediately.")
    }

    func testFsyncCallCountMatchesInterval() async throws {
        // Interval = 3
        let db = try VectorDatabase(
            dimension: 2, metric: .dotProduct, path: dbURL, walFlushInterval: 3)
        ...
        XCTAssertEqual(
            wal.fsyncCallCount - initialFsyncCount, 2,
            "fsync should be called exactly 2 times for 7 inserts with interval 3")
    }

    func testBatchInsertPerformsOnlyOneFsync() async throws {
        let db = try VectorDatabase(
            dimension: 2, metric: .dotProduct, path: dbURL, walFlushInterval: 1)
        try await db.save()
        ...
        XCTAssertEqual(
            wal.fsyncCallCount - initialFsyncCount, 1,
            "batchInsert must perform exactly 1 fsync after completing, regardless of item count")
