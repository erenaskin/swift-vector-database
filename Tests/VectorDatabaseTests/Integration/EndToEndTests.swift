...
final class EndToEndTests: XCTestCase {
    ...
    func testEndToEndMixedScenario() async throws {
        ...
        if let ep2 = ep2 {
            ...
            XCTAssertFalse(
                ep2Neighbors.contains("doc5"), "Deleted node should not appear in neighbors")
            XCTAssertFalse(
                ep2Neighbors.contains(ep1), "Deleted entry point should not appear in neighbors")
        }
        ...
        let dbReloaded = try VectorDatabase(dimension: 2, metric: .dotProduct, path: dbURL)
        ...
        let dbMapped = try VectorDatabase(dimension: 2, metric: .dotProduct, path: dbURL)
        for i in 2006...10000 {
