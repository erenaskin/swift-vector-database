...
final class ReadAPITests: XCTestCase {
    ...
    func testGetVectorExactMatch() async throws {
        let db = try VectorDatabase(dimension: 3, metric: .euclidean)
    ...
    func testPaginationAndDeletion() async throws {
        ...
        XCTAssertEqual(
            allRemaining, ["vec0", "vec1", "vec3", "vec4", "vec5", "vec6", "vec8", "vec9"])
    ...
    func testHNSWPaginationAndDeletion() async throws {
        ...
        // bir constructor ile deneyebiliriz ama VectorDatabase threshold'u parametre almıyor.
