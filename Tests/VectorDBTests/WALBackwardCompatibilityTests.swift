...
final class WALBackwardCompatibilityTests: XCTestCase {
    ...
    override func setUp() {
        super.setUp()
        fileURL = FileManager.default.temporaryDirectory.appendingPathComponent(UUID().uuidString).appendingPathExtension("wal")
    }
    ...
    func testLegacyWALParsing() throws {
        ...
        // Assert Record 1 (Legacy Insert)
        XCTAssertEqual(records[0].opcode, .insert)
        XCTAssertEqual(records[0].internalID, 42)
        XCTAssertEqual(records[0].timestamp, 1000)
        XCTAssertEqual(records[0].vectorData, [1.0, 2.0])
        XCTAssertNil(records[0].metadata)
        
        // Assert Record 2 (Legacy Delete)
        XCTAssertEqual(records[1].opcode, .delete)
        XCTAssertEqual(records[1].internalID, 42)
        XCTAssertEqual(records[1].timestamp, 1001)
        XCTAssertNil(records[1].vectorData)
        XCTAssertNil(records[1].metadata)
        
        // Assert Record 3 (New InsertWithMetadata)
        XCTAssertEqual(records[2].opcode, .insertWithMetadata)
        XCTAssertEqual(records[2].internalID, 43)
        XCTAssertEqual(records[2].timestamp, 1002)
        XCTAssertEqual(records[2].vectorData, [3.0, 4.0])
        XCTAssertEqual(records[2].metadata?["k"], "v")
    }
}
