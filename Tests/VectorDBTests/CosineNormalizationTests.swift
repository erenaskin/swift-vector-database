...
final class CosineNormalizationTests: XCTestCase {
    ...
    func testCosineNormalizationOnInsertAndSearch() async throws {
        let db = try VectorDB(dimension: 2, metric: .cosine, path: nil)
    ...
    func testWALRecoveryPreservesNormalizedVectors() async throws {
        ...
        // 1. Live index
        do {
            let db = try VectorDB(dimension: 2, metric: .cosine, path: dbURL)
            ...
            // since VectorDB.insert doesn't fsync yet (Task 3).
            let mirror = Mirror(reflecting: db)
            if let pm = mirror.children.first(where: { $0.label == "persistenceManager" })?.value as? PersistenceManager {
                try pm.wal?.fsync()
            }
        }
        
        // 2. Recovered index
        let recoveredDB = try VectorDB(dimension: 2, metric: .cosine, path: dbURL)
        
        let recoveredMirror = Mirror(reflecting: recoveredDB)
        guard let recoveredEngine = recoveredMirror.children.first(where: { $0.label == "engine" })?.value as? AnyObject else {
            XCTFail("Could not access engine")
            return
        }
        
        // Use perform method or cast. Engine is internal but we have @testable import VectorDB
        ...
        var finalVec1 = vec1
        let recoveredResults = finalVec1.withUnsafeMutableBufferPointer { buf -> [(id: Int32, score: Float)] in
        ...
        for (live, recovered) in zip(liveResults.sorted(by: { $0.score > $1.score }), recoveredResults.sorted(by: { $0.score > $1.score })) {
    ...
    func testNearZeroVectorThrowsInvalidVector() async throws {
        ...
        do {
            ...
            XCTFail("Expected VectorDBError.invalidVector to be thrown")
        } catch VectorDBError.invalidVector {
            // Success
        } catch {
    ...
    func testDotProductAndEuclideanMetricsNotNormalized() async throws {
        // DotProduct
        let dbDot = try VectorDB(dimension: 2, metric: .dotProduct, path: nil)
        let vec1: [Float] = [3.0, 4.0]
        let vec2: [Float] = [6.0, 8.0]
        ...
        // Euclidean
        let dbEuclid = try VectorDB(dimension: 2, metric: .euclidean, path: nil)
