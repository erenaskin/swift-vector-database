...
final class MetadataUpdateTests: XCTestCase {
    ...
    func testUpdateMetadataBasic() async throws {
        let db = try VectorDB(dimension: 2, metric: .euclidean)
    ...
    func testUpdateMetadataCrashRecovery() async throws {
        ...
        // save ile kalıcılaşıyor). VectorDB tasarımı gereği tam crash durumunda IDMap 
    ...
    func testBackwardCompatibilityWithOldWAL() throws {
        ...
        let walURL = dbURL.appendingPathExtension("wal")
        let fh = try FileHandle(forWritingTo: FileManager.default.createFile(atPath: walURL.path, contents: nil) ? walURL : walURL)
        defer { try? fh.close() }
        ...
        XCTAssertEqual(records[0].internalID, 10)
        XCTAssertEqual(records[0].vectorData, [0.5, 0.5])
        
        XCTAssertEqual(records[1].opcode, .delete)
        XCTAssertEqual(records[1].internalID, 10)
